...
from speech_client_origin import *
import numpy as np
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default=False,
        help="Enable verbose output",   # 是否启动详细输出
    )
    parser.add_argument(
        "-u",
        "--url",
    ...
    parser.add_argument(
        ...
        required=False,
        default=16,
        help="chunk size default is 16",    #一秒里分成16000份，16000/400=40块，1块里有16帧
    )
    parser.add_argument(
        "--context",
        type=int,
    ...
    num_workers = multiprocessing.cpu_count() // 2  #整除，向下取整，9//2=4
    ...
    def single_job(client_files):
        with grpcclient.InferenceServerClient(
            ...
            # triton_client.load_model("decoder")                 #只要使用了一次，把模型加载起来后就不需要这两个load_model了，只有在流式仓库的时候使用，非流式没有decoder
